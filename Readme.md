...
## Локальный запуск
Приложение использует ES-модули и `fetch`, поэтому открывать `index.html` как файл нельзя — нужен HTTP-сервер:
```bash
python3 -m http.server 8000
# http://localhost:8000
```
Первый запуск загружает каталог BrickLink в IndexedDB (~20 секунд), последующие — около секунды.
