...
| colorId    | `Color ID`, `ColorID`, `Color Code`, `Color` | int/string | ✅ |

Правила значения:
- `partId`: `trim()`, непустая строка, без смены регистра.
- `colorId`: целое `>= 0` или строка (название цвета для конвертации в ID).
...
2. Если `colorId` — строка (название цвета), найти соответствующий ID в справочнике Colors.

### Шаг 2. Дедупликация
- Ключ связи = пара `(partId, colorId)`.
- Результат — множество уникальных связей.
...
- **part_to_colors**: `Dict[str, List[int]]` — сгруппировать связи по `partId`.  
...
- Конвертация названий цветов в ID через справочник Colors.